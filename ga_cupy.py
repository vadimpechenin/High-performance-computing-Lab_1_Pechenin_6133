...
def mutation(offspring_crossover,const):
    ...
    random_value = cp.random.uniform(-const/4, const/4, offspring_crossover.shape[0]).T
...
def plot_res(massivs_of_x,massivs_of_y,massivs_of_y_points):
    ...
    #нулевого
    ax.plot(massivs_of_x[0], massivs_of_y[0], color='grey', label=r'$d_1$')
    #рисование графика зависимости второго столбца от
    #нулевого
    ax.scatter(massivs_of_x, massivs_of_y_points, label=r'$d_2$')
    ax.legend(loc='best')
    fig.show()
    fig.savefig('Результат генетического алгоритма с GPU.jpeg', dpi=300)